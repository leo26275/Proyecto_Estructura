...
public class ArbolBBReferencia<T> extends ArbolBinario {
        public ArbolBBReferencia() {
        super();
    }
    ...
    private NodoArbol insertar(T dato, NodoArbol r) {
        ...
        } else if (super.compareReferencia(dato, r) < 0) {
            NodoArbol izq;
    ...
    public NodoArbol buscar(String dato, NodoArbol r) {
        if (r == null) {
            return null;
        } else if (dato.compareToIgnoreCase(((Referencia) r.getDato()).getNombre()) == 0) {
            return r;
        } else {
            if (dato.compareToIgnoreCase(((Referencia) r.getDato()).getNombre()) < 0) {
                NodoArbol izq;
